...
export async function getServerSideProps(context) {
  ...
  data.trainPeriod = await objectFlat(train.trainPeriod,'train_period_id','train_period_name');
  data.time = await objectFlat(time.time,'time_id','time_name');
  data.classType = await objectFlat(classType.classType,'class_type_id','class_type_name');
  data.carType = await objectFlat(carType.carType,'car_type_id','car_type_name');
  data.source = await objectFlat(source.source,'source_id','source_name');
  data.postCode = objectFlat(postCode.postCode, 'post_code_name', 'post_code_addr');
  data.gender = {1: '男', 2: '女'};
