...
const getUser = async function (filter, pagination) {
	...
	let SELECT = `Select users.*, tp.*, tb.teacher_id, tb.time_id FROM users left join train_book as tb on tb.train_book_id = users.train_book_id left join train_period as tp on tp.train_period_id = tb.train_period_id where users.is_delete = false order by users.create_time desc`;
