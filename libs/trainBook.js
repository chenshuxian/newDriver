...
const getTrainBook = async function (filter, pagination) {
	...
	let prismaArgs = {};
	let queryTotal =
		'SELECT count(*) as total FROM train_book as tb left join users as u on tb.train_book_id = u.train_book_id where u.train_book_id is null';
	let queryId =
		'SELECT tb.train_book_id  FROM train_book as tb left join users as u on tb.train_book_id = u.train_book_id where u.train_book_id is null';
	...
	for await (const tp of trainPeriod) {
		...
		for await (const teach of teacher) {
			...
			for await (const t of time) {
				...
				data.push({
					train_period_id: tp.train_period_id,
...
const getBookingCount = async function (pid) {
	let count;
	const COUNT = `SELECT count(*) as count FROM train_book where train_period_id = '${pid}' 
	and train_book_id in (select train_book_id from users where train_book_id is not null)`;
	[{ count }] = await prisma.$queryRawUnsafe(`${COUNT}`);
	return count;
};

const getTrainBookIdForBook = async function (pid) {
	let train_book_id, count;
	const SQL = `SELECT train_book_id FROM train_book where train_period_id = '${pid}' 
  and train_book_id not in (select train_book_id from users where train_book_id is not null) limit 1`;
	count = await getBookingCount(pid);
	try {
		[{ train_book_id }] = await prisma.$queryRawUnsafe(`${SQL}`);
	} catch (e) {
		console.log(`getTrainBookIdForBook err: ${JSON.stringify(e)}`);
	}

	return { train_book_id, count };
};
