...
export const CustomerListResults = ({data}) => {
  ...
  const editUser = React.useCallback((row) => async () => {
    ...
    row.teacher_id = row.teacher_id === null ? getFirstId(data.teacher) : row.teacher_id;
    row.user_born_mg = getToday(1,row.user_born);

    if(row.train_period_id === null){
      row.train_period_id =  getFirstId(data.trainPeriod)
      row.train_period_start = trainPeriodDetail[`${row.train_period_id}`].train_period_start;
      row.train_period_end = trainPeriodDetail[`${row.train_period_id}`].train_period_end;
      row.train_period_exam = trainPeriodDetail[`${row.train_period_id}`].train_period_exam;
    }

    const time = await getBookTime(row.train_period_id,row.teacher_id,row.user_id);
    setTrainTime(time);
  ...
  const columns = [
    ...
    { field: 'teacher_id', headerName: '教練',
    ...
    { field: 'user_gender', headerName: '性別', width: 80,
      valueFormatter: (params) => {
  ...
  return (
    <>
    <CustomerListToolbar addUser={addUser} trainPeriod={data.trainPeriod} thisPeriod={data.thisPeriod}/>
    <Box sx={{ mt: 3 }}>
