...
import { createdUser, updatedUser, getStudentNumber } from  '../../libs/front/user';
...
export default function NewFormDialog(props) {
    ...
    const source_id = '0641878c-5967-11ec-a655-528abe1c4f3a';
    const user_gender = '1';
    ...
    if (!submittedValues) {
      // 新增
        train_period_start = trainPeriodDetail[`${train_period_id}`].train_period_start;
        train_period_end = trainPeriodDetail[`${train_period_id}`].train_period_end;
        train_period_exam = trainPeriodDetail[`${train_period_id}`].train_period_exam;
    ...
    [
        ...
        {name: "time_id", label: "訓練時間", type: "select", list: trainTime, xs:4, required: true},
        {name: "train_period_start", label: "訓練起始", type: "text", xs:4, disabled:true},
        {name: "train_period_end", label: "訓練結束", type: "text", xs:4, disabled:true},
        {name: "train_period_exam", label: "考試日期", type: "text", xs:4, disabled:true},
        {name: "car_type_id", label: "汽車類別", type: "select", list: data.carType, xs:4},
        {name: "class_type_id", label: "課程類別", type: "select", list: data.classType, xs:4},
        {name: "source_id", label: "報名來源", type: "select", list: data.source, xs:4},
        {name: "user_stu_num", label: "學號", type: "text", xs:4, required: true},
        {name: "user_id", label: "身份證", type: "text", xs:4, required: true, inputProps:{maxLength:10}},
        {name: "user_name", label: "姓名", type: "text", xs:4, required: true},
        {name: "user_gender", label: "性別", type:"select", list: data.gender, xs:2},
        {name: "user_born", label: "出生年月", type:"date", xs:6},
        {name: "user_born_mg", label: "民國年月", type:"text", xs:4, disabled:true},
        {name: "user_tel", label: "電話", type: "text", type:'tel', xs:4, inputProps:{maxLength:10}},
        {name: "user_email", label: "電子信箱", type:"email", xs:8, required: true},
 ...
 const onSubmit = async (values, form) => {
  ...
  submitData.train_book_id = await getBookId(train_period_id ,teacher_id, time_id);
  submitData.user_password = values.user_born_mg.replaceAll('-','');
  // train_period_id = values.train_period_id;

  const getUIData = (data) => {
    data.teacher_id = teacher_id;
    data.train_period_name = trainPeriodDetail[`${train_period_id}`].train_period_name;
  ...
  if(ADD) {
    ...
    if (result) {
      setRows((prevRows) => {
          let data = result.data;
          const rowToUpdateIndex = data.user_id;
          data = getUIData(data);
          return prevRows.map((row, index) => {
            // console.log(`${row.user_id} vs ${rowToUpdateIndex}`)
            return row.user_id == rowToUpdateIndex ? {...row, ...data} : row
          });
      });
      handleClose(); 
      window.alert('修改完成');
              ...
              <Grid container alignItems="center" spacing={2} className={classes.root}>
                {
                    fieldList.map((el,i) => {
                        if(el.type === 'select') {
                            if(values.train_period_start && el.name == 'train_period_id'){
